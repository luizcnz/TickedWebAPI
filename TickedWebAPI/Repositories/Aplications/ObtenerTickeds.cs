...
namespace TickedWebAPI.Repositories.Aplications
{
    public class ObtenerTickeds : IGetData
    {
        ...
        public ObtenerTickeds (TickedContext tickedContext)
        {
        ...
        public async Task<IActionResult> GetData()
        {
            ...
            try
            {
                var result = await _tickedContext.Tickeds.FromSqlRaw("exec {0}", procedureName).ToListAsync();
                if (result.Count == 0)
                {
